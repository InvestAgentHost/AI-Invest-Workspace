...
| `--securities` | get* | 证券逗号分隔：代码或名称（仅 **A 股**）。 |
| `--securities-file` | get* | CSV 含 `security_code` 列。 |
| `-sd` / `--start-date` | 否 | get 模式开始日期 `yyyy-MM-dd`，默认近一年。 |
| `-ed` / `--end-date` | 否 | get 模式结束日期，默认今天。 |
| `-p` / `--params` | 否 | JSON 字典，见下文「参数 `-p`」；请使用指标编码与参数编码（`paramKey`）。 |
...
- 使用 `--indicators-file` 时会读取 `indicator_params` 列；若同时传入 `-p`，**以 `-p` 为准**覆盖文件中的同名参数。
- 时序长度（日历天数）**大于** `min(指标数, 证券数)` → 使用 **时序接口**：
  - 证券数更多：单指标 × 多证券，按指标分批请求。
  - 否则：多指标 × 单证券，按证券分批请求。
- 否则使用 **截面接口**（`date` 取 `-ed`）。
