---
name: gangtise-file
description: "在 Gangtise 文件中心按类型/日期/证券等条件检索文档并可选下载全文，覆盖：国内研究报告与研报图片、外资研报、首席观点、外资机构观点与外资独立观点、会议纪要、公众号资讯、公司公告（A股/港股/美股）、管理层讨论与分析（半年报/年报/业绩会）、投资者问答、投研日程（路演/调研/线下策略会/论坛/财报日历）。返回文件 ID、元数据与摘要，适合“先定位/筛选再下载核验”；深入阅读与引用段落内容请优先用 `gangtise-kb`。"
metadata:
  version: 1.6.9
  author: Gangtise
  category: openapi
...
该技能会查询 Gangtise 文件中心，检索多种文档类型，例如研究报告、研报图片、投资者问答、外资研报、外资机构/独立观点、公司公告、会议纪要、首席观点、公众号资讯与投研日程（路演、调研、线下策略会、论坛、财报日历）等。它会返回文件 ID，并附带关键元数据与简短摘要；这些命中结果基于检索匹配与字段归纳，强调专业性、准确性与覆盖全面，同时提供按“类型 + ID”下载完整文件的方法。注意：由于 API 限制，大多数结果最多限制为 100 条；多数情况下建议将结果数量限制在 10 条以内。
