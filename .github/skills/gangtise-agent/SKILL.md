...
- `stock-one-line-summary`：个股一句话总结（A 股/港股；支持批量证券或 `aShares` / `hkStocks` 全市场）
...
| `--securities` | 条件必填 | 多个证券名称或代码，逗号分隔（如 `贵州茅台,比亚迪`）；与 `--security` 可同时使用并去重合并。用于同上需证券的 agent 类型。`stock-one-line-summary` 另支持全市场标识 `aShares`（全部 A 股）或 `hkStocks`（全部港股），不可与具体证券混传。 |
...
```bash
python3 scripts/agents.py -a stock-one-line-summary --securities aShares
```
